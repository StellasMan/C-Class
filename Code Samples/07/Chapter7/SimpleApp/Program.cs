...
namespace SimpleApp
{
	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			RunSequential();
			Console.WriteLine("We're done in {0:#,###} ms!\n", sw.ElapsedMilliseconds);

			sw.Restart();
			Console.WriteLine("Running With Threads");
			RunWithThreads();
			Console.WriteLine("We're done in {0:#,###} ms!\n", sw.ElapsedMilliseconds);
			...
			sw.Restart();
			Console.WriteLine("Running in Thread Pool with Events");
			RunInThreadPoolWithEvents();
			Console.WriteLine("We're done in {0:#,###} ms!\n", sw.ElapsedMilliseconds);

			Utils.CommonFunctions.WaitForKeyWhenDebugging();
		}
